...
export function Topbar() {
  ...
  return (
    <header className="flex h-16 items-center justify-between bg-white px-6 z-10 select-none shadow-[0_2px_12px_-4px_rgba(89,62,46,0.04)]">
      {/* Search / Context Area */}
      <div className="flex items-center gap-4">
        <div className="relative hidden md:flex items-center">
          ...
          <input
            ...
            className="h-9 w-80 rounded-xl bg-[#F2E6D8]/50 pl-9 pr-4 text-xs text-[#2d1f17] placeholder:text-[#705849] focus:bg-white focus:ring-2 focus:ring-[#555934]/20 focus:outline-none transition-all shadow-2xs"
            readOnly
      ...
      <div className="flex items-center gap-3">
            ...
            <div className="absolute right-0 mt-2 w-80 rounded-2xl bg-white p-2 shadow-card-elevated z-50 animate-in fade-in zoom-in-95 duration-100">
              ...
              <div className="divide-y divide-[#F2E6D8] max-h-72 overflow-y-auto">
                {notifications.map((item) => (
                  <div
                    key={item.id}
                    className={`p-3 text-left rounded-lg transition-colors hover:bg-[#F2E6D8]/60 ${
        ...
        <div className="relative" ref={menuRef}>
          ...
          >
            <div className="flex h-7 w-7 items-center justify-center rounded-md bg-[#555934] text-white text-xs font-bold shadow-2xs">
              AS
            </div>
            <div className="hidden md:flex flex-col text-left">
              <span className="text-xs font-semibold text-[#2d1f17] leading-tight">
                Amit Sharma
              </span>
              <span className="text-[10px] text-[#705849] -mt-0.5">
          ...
          {menuOpen && (
            <div className="absolute right-0 mt-2 w-56 rounded-2xl bg-white p-2 shadow-card-elevated z-50 animate-in fade-in zoom-in-95 duration-100">
              <div className="px-3 py-2 border-b border-[#F2E6D8]">
                <p className="text-xs font-bold text-[#2d1f17]">Amit Sharma</p>
                <p className="text-[11px] text-[#705849] truncate">
                  amit.sharma@mospi.gov.in
                </p>
                <div className="mt-1.5 inline-flex items-center gap-1 rounded-md bg-[#555934]/12 px-2 py-0.5 text-[9px] font-semibold text-[#555934]">
                  SSS Cadre • L1-L5 Track
                </div>
              </div>
